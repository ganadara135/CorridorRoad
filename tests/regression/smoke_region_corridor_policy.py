...
def run():
    ...
    _assert("src[" in package_summary and "region=1" in package_summary and "full=1" in package_summary and "mode[-=1, split_only=1]" in package_summary and "contract[section_profiles=2]" in package_summary, "SegmentPackageSummary mismatch for region-driven split/skip")
    ...
    _assert("corridorRule=region_aware" in status, "Corridor status missing region-aware token")
    _assert("corridorModes=region|" in status, "Corridor status missing combined region mode summary")
    _assert("regionCorridorModes=split_only=1, skip_zone=1" in status, "Corridor status missing region mode summary")
    _assert("skipZones=1" in status, "Corridor status missing skip-zone summary")
    ...
    _assert("segmentDrivers=region=2" in status, "Corridor status missing segmentDrivers token")
    _assert("segmentDriverSources=" in status and "region=1" in status, "Corridor status missing usable segmentDriverSources token")
    ...
    _assert("segmentDisplay=" in status and "region:BASE_SPLIT:split_only@" in status and "contract=section_profiles" in status, "Corridor status missing usable segmentDisplay token with contract source")
    ...
    _assert("diagPolicy=info" in status, "Corridor status missing diagPolicy token")
    _assert("diagClasses=source:ok, connectivity:ok, packaging:info, policy:info" in status, "Corridor status missing diagnostic class summary")
