...
"""
Cross Section Editor PH-5 typical cross-slope edit-plan apply smoke test.

Run in FreeCAD Python environment:
    FreeCADCmd -c "exec(open(r'tests\\regression\\smoke_cross_section_editor_typical_slope_edit_plan_apply.py', encoding='utf-8').read())"
"""
...
def run():
    ...
    doc = App.newDocument("CRCrossSectionEditorTypicalSlopeEditPlanApply")
    ...
    try:
        ...
        panel.spin_transition_in.setValue(0.0)
        panel.spin_transition_out.setValue(0.0)
        panel._apply_editor_edit()

        plan = getattr(sec, "CrossSectionEditPlan", None)
        _assert(plan is not None, "Editor apply should create CrossSectionEditPlan for typical slope edit")
        records = list(CrossSectionEditPlan.records(plan) or [])
        _assert(len(records) == 1, "Typical slope editor apply should create one record")
        rec = dict(records[0] or {})
        _assert(str(rec.get("TargetId", "") or "") == "LANE-L", "Typical slope record target id mismatch")
        _assert(str(rec.get("SourceScope", "") or "") == "typical", "Typical slope record source scope mismatch")
        _assert(str(rec.get("Parameter", "") or "") == "cross_slope_pct", "Typical slope record parameter mismatch")
        _assert(abs(float(rec.get("Value", 0.0) or 0.0) - 4.0) < 1.0e-6, "Typical slope record value mismatch")
        ...
        _assert(str(lane.get("source", "") or "") == "cross_section_edit", "Edited LANE-L slope row should report cross_section_edit source")
        _assert(abs(float(lane.get("slope", 0.0) or 0.0) - 4.0) < 1.0e-6, "Edited LANE-L slope should be 4.0%")

        panel._teardown()
        print("[PASS] Cross Section Editor PH-5 typical cross-slope edit-plan apply smoke test completed.")
    finally:
