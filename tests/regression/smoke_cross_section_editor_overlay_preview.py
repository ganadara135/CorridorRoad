...
"""
Cross Section Editor PH-7 overlay-preview smoke test.

Run in FreeCAD Python environment:
    FreeCADCmd -c "exec(open(r'tests\\regression\\smoke_cross_section_editor_overlay_preview.py', encoding='utf-8').read())"
"""
...
def run():
    ...
    doc = App.newDocument("CRCrossSectionEditorOverlayPreview")
    ...
    try:
        ...
        _assert(_debug_row("selected|", rows), "Selected overlay row should exist")
        _assert(not _debug_row("preview|", rows), "Preview overlay should not exist when value matches source")

        panel.spin_width.setValue(8.0)
        panel._refresh_editor_apply_state()
        rows = list(getattr(panel, "_editor_overlay_debug_rows", []) or [])
        selected = _debug_row("selected|", rows)
        preview = _debug_row("preview|", rows)
        _assert(preview, "Preview overlay row should exist when a pending width edit is present")
        _assert("parameter=width" in preview, "Preview overlay should report width parameter")
        _assert("old=6.000" in preview and "new=8.000" in preview, "Preview overlay should report old/new width values")

        selected_x0 = float(_parse_field(selected, "x0", "0.0") or 0.0)
        preview_x0 = float(_parse_field(preview, "x0", "0.0") or 0.0)
        _assert(preview_x0 < selected_x0, "Left-side width increase should push preview overlay outward")

        panel.spin_width.setValue(6.0)
        panel._refresh_editor_apply_state()
        rows = list(getattr(panel, "_editor_overlay_debug_rows", []) or [])
        _assert(not _debug_row("preview|", rows), "Preview overlay should clear after resetting to the source value")

        panel._teardown()
        print("[PASS] Cross Section Editor PH-7 overlay-preview smoke test completed.")
    finally:
        try:
