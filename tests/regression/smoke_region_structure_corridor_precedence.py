...
def run():
    ...
    _assert("segmentRows=" in status, "CorridorLoft status missing segmentRows token")
    ...
    _assert("segmentSources=structure+region" in status, "CorridorLoft status missing segmentSources token")
    _assert("segmentKinds=segment=2, skip=1" in status, "CorridorLoft status missing segmentKinds token")
    _assert("segmentDrivers=mixed=2" in status, "CorridorLoft status missing segmentDrivers token")
    _assert("segmentDriverSources=" in status and "structure=1" in status, "CorridorLoft status missing usable segmentDriverSources token")
    ...
    _assert("segmentDisplay=" in status and "structure:STR_SPLIT:split_only@" in status and "contract=section_profiles" in status, "CorridorLoft status missing usable segmentDisplay token with contract source")
    ...
    _assert("diagPolicy=warn" in status, "CorridorLoft status missing diagPolicy token")
    _assert("diagClasses=source:ok, connectivity:ok, packaging:info, policy:warn" in status, "CorridorLoft status missing diagnostic class summary")
