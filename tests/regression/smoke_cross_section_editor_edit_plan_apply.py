...
def run():
    ...
    try:
        ...
        _assert(not bool(panel.btn_apply_edit.isEnabled()), "Apply should stay disabled until Preview Impact is current")
        panel.btn_preview_impact.click()
        _assert(bool(panel.btn_apply_edit.isEnabled()), "Apply should enable after Preview Impact is refreshed")
        panel._apply_editor_edit()
        current_after_first = dict(panel._current_editor_segment() or {})
        _assert(str(current_after_first.get("id", "") or "").strip().upper() == "L10", "Editor should keep the same target selected after apply")
        _assert(str(current_after_first.get("side", "") or "").strip().lower() == "left", "Editor should keep the same target side after apply")
        _assert(not bool(panel.btn_apply_edit.isEnabled()), "Apply should become stale again after apply refreshes the editor")
        _assert("Preview is stale. Run Preview Impact again before applying." in str(panel.lbl_apply_state.text() or ""), "Apply state should explain stale preview after apply")
        ...
        _assert(not bool(panel.btn_apply_edit.isEnabled()), "Changing the edit target should invalidate the preview")
        panel.btn_preview_impact.click()
        _assert(bool(panel.btn_apply_edit.isEnabled()), "Apply should enable again after station-only preview refresh")
        panel._apply_editor_edit()
