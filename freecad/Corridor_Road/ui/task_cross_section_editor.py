...
class CrossSectionEditorTaskPanel(CrossSectionViewerTaskPanel):
    ...
    def _build_editor_panel(self):
        ...
        self.cmb_edit_parameter.addItems(["Width", "Slope %", "Region Side Policy", "Region Daylight Policy"])
        ...
        self.spin_width.setRange(0.0, 1000000.0)
        self.spin_width.setSingleStep(0.100)
        main.addWidget(QtWidgets.QLabel("Width Value"))
        ...
        main.addWidget(QtWidgets.QLabel("Slope Value"))
        ...
        self.spin_width.valueChanged.connect(self._refresh_editor_apply_state)
        self.spin_slope.valueChanged.connect(self._refresh_editor_apply_state)
        self.cmb_region_policy.currentIndexChanged.connect(self._refresh_editor_apply_state)
    ...
    def _refresh_editor_width_control(self, seg):
        ...
        try:
            display_width = float(dict(seg or {}).get("display_span", dict(seg or {}).get("span", 0.0)) or 0.0)
            self.spin_width.setValue(max(0.0, display_width))
            self.spin_width.setSuffix(f" {self._display_unit_label()}")
            target, _message = self._editor_apply_target(seg) if self._selected_edit_parameter() == "width" else (None, "")
            self.spin_width.setEnabled(self._selected_edit_parameter() == "width" and bool(target))
        finally:
    ...
    def _refresh_editor_slope_control(self, seg):
        ...
        try:
            target, _message = self._editor_apply_target(seg) if self._selected_edit_parameter() == "slope" else (None, "")
            old = float(dict(target or {}).get("old_slope", dict(target or {}).get("old_value", 0.0)) or 0.0)
            self.spin_slope.setValue(old)
            self.spin_slope.setEnabled(self._selected_edit_parameter() == "slope" and bool(target))
        finally:
    ...
    def _refresh_editor_apply_state(self, *_args):
        ...
        if hasattr(self, "lbl_apply_state"):
            if enabled:
                ...
                self.lbl_apply_state.setText(f"{phase} safe apply enabled: {target.get('description', 'edit will be written to the linked source')}.")
            else:
                self.lbl_apply_state.setText(message or "Safe apply is not available for this target.")
    ...
    def _ph5_edit_plan_target(self, seg):
        ...
        if parameter not in ("width", "slope"):
            return None, "PH-5 edit-plan apply currently supports Width and Slope % only."
        scope = str(self.cmb_edit_scope.currentText() or "")
        ...
        if source_scope == "side_slope":
            ...
            if parameter not in ("width", "slope"):
                return None, "PH-5 typical-component edit-plan apply currently supports Width and Slope % only."
            if not target_id:
        ...
        if parameter == "width":
            ...
            if not math.isfinite(value) or value < 0.0:
                return None, "Width value must be finite and non-negative."
        ...
        resolved_parameter = "width" if parameter == "width" else ("cross_slope_pct" if source_scope == "typical" else "slope_pct")
        return {
            "phase": "PH-5",
            "kind": "edit_plan",
    ...
    def _editor_validation_rows(self, *, seg=None, target=None, message=""):
        ...
        if parameter == "width":
            rows.append(f"validation|phase={phase}|level=ok|code=value|message=Width is finite and non-negative.")
        elif parameter == "slope":
            rows.append(f"validation|phase={phase}|level=ok|code=value|message=Slope percent is finite.")
        else:
            rows.append(
