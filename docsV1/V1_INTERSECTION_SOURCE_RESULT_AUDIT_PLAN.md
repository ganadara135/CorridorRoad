...
| `IntersectionSlopeFaceLoopResult` | `IntersectionSlopeFaceLoopRow` | Captures loop-owned Slope Face result and diagnostics. | Needs source-edge-family completeness status in diagnostics. |
...
| --- | --- | --- | --- | --- | --- | --- |
| INT-SRC-001 | Planned | source | `models/source/intersection_model.py`, object persistence | Add or promote `IntersectionAnchorRow` with detected/manual/locked status and station refs. | Source roundtrip test for anchor rows. | Create starter T, lock anchor, reopen panel, confirm anchor remains fixed. |
| INT-SRC-002 | Planned | source | `intersection_model.py`, `cmd_intersection_presets.py` | Add leg source status fields or equivalent validation for Profile, Centerline3D, Region, span source, and approval. | Contract test for missing leg refs producing source diagnostics. | T/Cross/Y wizard shows missing leg refs before preview. |
| INT-SRC-003 | Planned | source | `intersection_model.py`, region linkage helpers | Add intersection-owned control-area intent before Region linkage. | Source validation test for `intersection_owned`, `region_derived`, `preset_default`. | Edit control area intent and confirm Region mismatch warning. |
| INT-SRC-004 | Planned | source | `intersection_model.py` | Add corner source rows and corner-based curb-return policy refs. | Curb-return source roundtrip and topology/edge result test. | Confirm curb return attaches to named corner, not only global radius. |
| INT-SRC-005 | Planned | source | `intersection_model.py`, Assembly/Subassembly bridge | Add edge-family policy rows derived from Assembly/Subassembly contracts with approval status. | Edge-family source test for lane, shoulder, gutter, curb, sidewalk, ditch, median, side_slope. | Edge Network preview shows approved/default edge families. |
| INT-SRC-006 | Planned | source | `intersection_model.py` | Add lane connection policy rows. | Topology/edge test for through/turn/merge/terminate rows. | Cross intersection shows lane connection completeness before surface preview. |
| INT-SRC-007 | Planned | source/evaluation | `intersection_model.py`, `intersection_evaluation_service.py` | Expand vertical/grading source policy with controlling profile, crown, tie-in, crossfall transition, low-point strategy. | Grading context result includes vertical policy refs and warnings. | Surface-zone preview warns when vertical policy is incomplete. |
| INT-SRC-008 | Planned | source/evaluation | `intersection_model.py`, drainage source bridge | Expand drainage intent source and distinguish hints from accepted drainage design. | Drainage hint test reports hint-only vs accepted source. | Drainage-sensitive sample shows inlet/low-point handoff status. |
| INT-RES-001 | Planned | result | intersection result dataclasses | Add row-level source status and lineage refs to topology, edge network, surface zone, grading, drainage, and slope loop rows. | Result contract tests check `source_status` and source refs. | Build Corridor review lists accepted/default/fallback status. |
| INT-EVAL-001 | Planned | evaluation | `intersection_evaluation_service.py` | Emit diagnostics for anchor, control-area intent, edge-family, lane connection, vertical, and drainage source gaps. | Diagnostic contract tests for each missing source class. | Wizard and Build Corridor show the same missing-source reason. |
| INT-UI-001 | Planned | presentation/source | `cmd_intersection_editor.py`, `cmd_intersection_presets.py` | Replace compact creation flow with staged source-completeness wizard. | UI smoke through command construction where feasible. | User can see incomplete stages before Apply. |
| INT-UI-002 | Planned | presentation/result | Intersections panel, Build Corridor, Cross Section Viewer | Add result preview sequence: source validation, topology, edge network, surface zones, grading, drainage, slope loops. | Preview command tests for status rows. | Confirm previews are read-only and source refs are visible. |
| INT-CONS-001 | Planned | result/output | `applied_section_service.py`, `cmd_generate_applied_sections.py` | Applied Sections carry source status and fallback diagnostics for intersection context. | AppliedSectionSet contract test for intersection context status. | Cross Section Viewer shows station/source/fallback status. |
| INT-CONS-002 | Planned | output/presentation | `cmd_build_corridor.py` | Label legacy patch and fallback output paths as `legacy_output` or `inferred_fallback`. | Guided Review contract test for status labels. | Build Corridor does not silently hide missing source intent. |
| INT-CONS-003 | Planned | output | `cmd_watertight_solids.py` | Keep intersection patch solids transitional and diagnostic until accepted contracts are sufficient. | Handoff test confirms patch path reports transition status. | Watertight panel warns that patch body is not final Digital Twin quality. |
