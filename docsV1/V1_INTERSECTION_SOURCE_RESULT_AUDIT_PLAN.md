...
| Arm policy | yes | yes | yes | `IntersectionArmPolicyRow` exists. | Required | Mostly sufficient; should be reviewed per leg before edge-network result acceptance. |
...
| Edge Network contract diagnostics | edge family status, curb-return corner status, lane connection status, source policy refs, inferred/default edge rows | Corners / Curb Returns / Edge Families / Lane Connections steps |
