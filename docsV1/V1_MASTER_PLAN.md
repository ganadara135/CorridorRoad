...
- prepare terrain-inclusive road simulation models from validated corridor, terrain, drainage, structure, and Subassembly outputs
...
- solid outputs are for physical bodies with thickness, material, volume, or asset identity such as pavement layers, curbs, gutters, walls, bridge elements, culverts, pipes, and IFC/export bodies
- watertight solid outputs are topology-first results: closed semantic profiles, deterministic edge networks, face adjacency, shell closure, and validation must be established before Part solid geometry is accepted
- simulation-ready watertight output should be assembled from independently validated terrain, road-body, Subassembly, drainage, and structure solids before any optional final boolean composition is attempted
- watertight solid outputs must not be generated by treating generated open surfaces or viewer meshes as editable source geometry
...
| Structure solid output | topology-first where solid | Structure bodies need closed profiles or swept bodies, validation, volume, and exchange identity. |
| Drainage grading surface | geometry-first | Open ditch, swale, and channel grading surfaces are terrain-like review or analysis surfaces. |
| Drainage physical body | topology-first | Lined ditches, concrete channels, culverts, headwalls, inlets, and outlets need closed physical body topology. |
| Watertight Solid | topology-first | Watertight output requires closed profiles, deterministic edges, face adjacency, shell closure, and solid validation. |
| Quantity output | contract-first | Quantities must consume normalized source/result/output contracts and preserve traceable measurement basis. |
| Exchange output | contract-first | DXF, LandXML, IFC, JSON, and package exports should map from normalized contracts rather than runtime viewer state. |
| Review UI | contract-first presentation | Review panels should show source ownership, result diagnostics, and output state without becoming geometry editors. |
| AI Assist | semantic/contract-first | AI suggestions must reason over source contracts, diagnostics, and constraints, and require approval before changing design state. |

### 5.6 Explanation before automation
...
### 5.7 Viewer over direct output editing
...
5. Review the 3D Centerline baseline where Structures, Drainage, Applied Sections, and Build Corridor will share station/offset/elevation context
6. Define superelevation, drainage constraints, and design criteria
7. Author section templates, intersection assemblies, and ramp rules
8. Assign regions, junction policies, and localized rules
9. Add structures, drainage elements, and interaction rules
...
3. Select a Subassembly or rule-derived area
...
### 6.5 Terrain-inclusive simulation workflow

1. Build or import the existing-ground TIN.
2. Build the corridor network from accepted Alignment, Profile, Assembly, Region, Drainage, and Structure sources.
3. Review surface outputs such as existing ground, design surface, subgrade, daylight, drainage grading, and terrain interaction.
4. Open `Watertight Solids` after `Build Corridor`.
5. Generate independent watertight solid targets for road body, Subassembly layers, drainage bodies, and structure bodies.
6. Generate or derive terrain boundary solids where the target simulation requires a closed ground volume or domain boundary.
7. Run composition diagnostics for gaps, overlaps, interfaces, and non-manifold contact between terrain, road, drainage, and structure solids.
8. Create a simulation-ready output package from the validated independent solids.
9. Apply optional boolean union, subtraction, or clipping only after independent target validation has passed.

Simulation-ready output is not a new editable source model.

It is a downstream output assembled from validated source-owned results.
...
### 9.15 WatertightSolidModel

Responsibilities:

- define solid targets for whole-corridor, selected Region, drainage, structure, and Subassembly body outputs
- support terrain-inclusive whole-road simulation packages by collecting road-body, Subassembly, drainage, structure, and terrain-boundary solid outputs
- derive closed semantic section profiles from accepted Applied Sections and source references
- build topology-first station-direction edge networks, faces, shells, and validated solids
- preserve independent target identity before optional final composition so diagnostics, quantities, and exchange metadata remain traceable
- report composition diagnostics for target-to-target gaps, overlaps, contact edges, and boolean-readiness where practical
- preserve source lineage to Region, Assembly, Structure, Drainage, AppliedSectionSet, and CorridorModel inputs
- report watertight diagnostics such as open edges, non-manifold edges, cap failures, invalid normals, self-intersection warnings, and volume checks
- feed quantity, exchange, and simulation outputs only after validation status is reviewable

### 9.16 EarthworkBalanceModel
...
- `SubassemblySectionTemplate`: authored source definition
...
### 10.2 SubassemblySectionTemplate
...
2. user selects a Subassembly or problem area
...
13. `Exchange`
14. `AI Assist`
15. `Watertight Solids`

`Watertight Solids` should be the last toolbar stage. It should remain disabled until `Build Corridor Network` has produced the accepted corridor prerequisites needed for solid generation.
...
Subassembly identity and rule identity must be explicit and durable.
...
### 19.8 Phase H: watertight simulation output

- validate independent road-body, Subassembly, drainage, structure, and terrain-boundary solids
- add composition diagnostics for gaps, overlaps, non-manifold contacts, and boolean-readiness
- create simulation-ready output packages from accepted solid targets
- keep optional final boolean composition separate from independent target validation

### 19.9 Phase I: stabilization
...
13. `docsV1/V1_WATERTIGHT_SOLID_PLAN.md`
...
8. define watertight solid target, topology validation, and final toolbar workflow contracts
...
5. Does it help produce practical engineering outputs, simulation-ready solids, and exchange data?
